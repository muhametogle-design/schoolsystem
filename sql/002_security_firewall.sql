...
CREATE OR REPLACE FUNCTION app_is_state_role() RETURNS BOOLEAN AS $$
    SELECT app_current_role_name() = 'state_inspector';
$$ LANGUAGE sql STABLE;
...
CREATE POLICY financial_firewall ON tuition_rates
    ...
    WITH CHECK (school_id = app_current_school_id() AND NOT app_is_state_role());
...
GRANT SELECT ON private_schools, academic_years, school_classes, students,
                subjects, student_grades, live_attendance,
                daily_submission_logs, exam_submission_events,
                communication_logs, security_audit_log
TO state_readonly;
...
GRANT SELECT, INSERT, UPDATE, DELETE ON private_schools, academic_years, users,
    school_classes, students, subjects, student_grades, exam_submission_events,
    live_attendance, daily_submission_logs, communication_logs,
    tuition_rates, student_invoices, payment_transactions, security_audit_log
TO school_app;
...
-- 4c. Published exam marks cannot be silently reverted to draft
