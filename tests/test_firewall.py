...
FINANCIAL_ROUTES = [
    ("GET", "/api/school/billing/summary", None),
    ("GET", "/api/school/billing/tuition-rates", None),
    ("GET", "/api/school/billing/invoices", None),
    ("POST", "/api/school/billing/invoices", {"student_id": 1, "description": "x", "amount_due": 10}),
    ("POST", "/api/school/billing/invoices/1/payments", {"amount": 1, "payment_method": "Cash"}),
]
...
def test_state_violations_are_audited(client, auth_headers):
    client.get("/api/school/billing/summary", headers=auth_headers)
    with SessionLocal() as db:
        rows = (
            db.query(SecurityAuditLog)
            .filter(SecurityAuditLog.verdict == "BLOCKED", SecurityAuditLog.endpoint == "/api/school/billing/summary")
            .all()
        )
...
def test_teachers_blocked_from_financials(client, greenfield_teacher_headers):
    res = client.get("/api/school/billing/summary", headers=greenfield_teacher_headers)
...
def test_school_roles_blocked_from_state_portal(client, greenfield_manager_headers, greenfield_teacher_headers):
    for headers in (greenfield_manager_headers, greenfield_teacher_headers):
        assert client.get("/api/state/compliance-map", headers=headers).status_code == 403
        assert client.get("/api/state/students/search?q=x", headers=headers).status_code == 403
        assert client.post("/api/state/audit/run", headers=headers).status_code == 403
...
def test_tenant_isolation_between_schools(client, horizon_manager_headers, greenfield_manager_headers):
    ...
    gf = client.get("/api/school/students", headers=greenfield_manager_headers).json()["students"]
    hz = client.get("/api/school/students", headers=horizon_manager_headers).json()["students"]
    ...
    gf_classes = client.get("/api/school/classes", headers=greenfield_manager_headers).json()["classes"]
